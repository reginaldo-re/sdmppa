...
#'  **ES**, as an index. So,
#'  the values in the named list must reference rows in **ES** that can be joined
...
#' environmental space in **ES** to such that It does not contain *PP*, and avoids
#' common points in **PP** and **PAP**. There are two choices when a list is
#' passed: the list is an index referencing rows in **ES**, and the name of the
#' list must match a column in **ES**; or, it is an unnamed list containing data
...
#'
#' @export
pa_random <- function(ES, PP) {
  UseMethod("pa_random")
}


#' @export
pa_random.data.frame <- function(ES, PP = NULL) {
  if (checkmate::test_data_frame(PP)) {
    return(.pa_random_df(ES, PP))
  } else if (checkmate::test_list(PP)) {
    if (length(PP) == 1) {
      return(.pa_random_list(ES, PP))
    } else {
      return(.pa_random_df_list(ES, PP))
    }
  }
  ...
  cli::cli_abort(
    c(
      ...
      in a column of ES (or a list os lists)!"
    )
  )
}

.pa_random_df <- function(ES, PP) {
  ...
  return(.pa_random(ES, PP))
}

.pa_random_list <- function(ES, PP) {
  ...
  id <- names(PP)[1]
  if (is.null(id) || !(id %in% names(ES))) {
    cli::cli_abort(c("x", "ES does not contains PP variable name!"))
  }
  PP_joined <- ES |>
  ...
  PAP <- dplyr::lst(!!id := .pa_random(ES, PP_joined) |> purrr::pluck(id))
  return(PAP)
}

.pa_random_df_list <- function(ES, PP) {
  PP_list <- PP |>
    purrr::imap(\(x, name_x) {
      PPA_iter <- NULL
      ...
      if (checkmate::test_data_frame(x)) {
        ...
        PPA_iter <- possibly_pa_random(ES, x)
      } else {
        if (name_x %in% colnames(ES) &&
          ...
          PPA_iter <-
            possibly_pa_random(ES, dplyr::lst(!!name_x := x)) |>
            purrr::pluck(1)
        }
      }
      return(PPA_iter)
...
.pa_random <- function(ES, PP = NULL) {
  ...
  names_PP <- names(ES)
  names_PPP <- names(PP)
  names_both <- names_PPP[names_PPP %in% names_PP]
