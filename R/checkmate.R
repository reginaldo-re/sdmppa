...
check_function_cli <- function(...){
  return(checkmate::check_function(...))
}
...
make_assertion = function(x, res, var.name, collection) {
  if (!isTRUE(res)) {
    assert_string(var.name, .var.name = ".var.name")
    ...
    assert_class(collection, "AssertCollection", .var.name = "add")
    collection$push(
