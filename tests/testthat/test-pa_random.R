P <- vroom::vroom(test_path("testdata", "predictors.csv"), show_col_types = F)
PP <- vroom::vroom(test_path("testdata", "argncts_ln.csv"), show_col_types = F)
PP_predictors <- PP |> dplyr::select(-argncts_ln)
...
test_that(".pa_random - Test invalid P", {
  ...
  expect_error(
    matrix(nrow = 0, ncol = ncol(P)) |>
      data.frame() |>
      setNames(P) |> .pa_random(PP_predictors),
...
test_that(".pa_random - Test success case, one less variable", {
  ...
  checkmate::expect_data_frame(PAP, any.missing = F, all.missing = F, nrows = nrow(PP_predictors), ncols = ncol(P), null.ok = F)
  expect_equal(
    PAP |>
      dplyr::setdiff(PP_predictors) |>
      nrow(),
    nrow(PP)
  )
...
test_that("pa_random.data.frame - Test invalid PP", {
  ...
  expect_error(
    P |>
      .pa_random(PP_predictors |> dplyr::bind_rows(P)),
    "Assertion on 'PP' failed: Must have at most"
  )
  expect_error(
    P |>
      pa_random(list("cell_id"=c(P$cell_id, c(1)))),
    "failed: Must have length"
  )
  expect_error(
    P |>
      pa_random(list("cell_id"=c(PP_predictors$cell_id, c(NA)))),
    "failed: Contains missing values"
  )
  expect_error(
    P |>
      pa_random(list("cell_id"=NULL)),
    "PP must be a dataframe"
  )
  expect_error(
    P |>
      pa_random(list("cell_id_not_in_P"=PP_predictors$cell_id)),
    "P does not contains PP variable name!"
  )
})


test_that("pa_random.data.frame - Test success case, when intersection between PAP and PP (dataframe) is null, |PAP|==|PP|, and PAP in PP", {
  PAP <- P |>
    pa_random(PP_predictors)
...
test_that("pa_random.data.frame - Test success case, when intersection between PAP and PP (list) is null, |PAP|==|PP|, and PAP in PP", {
  expect_warning(
    P |>
      pa_random(list("cell_id"=c(PP_predictors$cell_id, c(1)))),
...
test_that("pa_random.data.frame - Test a list of dataframes and named lists", {
  ...
  checkmate::expect_data_frame(PAP[[1]], any.missing = F, all.missing = F, nrows = nrow(PP_predictors), ncols = ncol(P), null.ok = F)
  checkmate::expect_data_frame(PAP[[2]], any.missing = F, all.missing = F, nrows = nrow(PP_predictors), ncols = ncol(P), null.ok = F)
  expect_true(
    checkmate::test_vector(PAP[[3]], strict = T, any.missing = F, all.missing = F, null.ok = F, len = nrow(PP))
  )
